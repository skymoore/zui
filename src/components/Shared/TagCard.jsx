...
const DropdownToggle = styled(Typography)({
  color: '#1479FF',
  paddingTop: '1rem',
  fontSize: '0.8125rem',
  fontWeight: '600',
    ...
    <StyledCard raised>
      <StyledCardContent>
        ...
        <Stack direction="row" onClick={() => setOpen(!open)}>
          ...
          <DropdownToggle>{!open ? `Show more` : `Show less`}</DropdownToggle>
        </Stack>
        <Collapse in={open} timeout="auto" unmountOnExit>
          <Box sx={{ marginTop: '1rem' }}>
            <Grid container item xs={12} direction={'row'}>
              <Grid item xs={6} md={6}>
                <Typography variant="body1" sx={(theme) => ({ color: theme.palette.secondary.dark, fontSize: '1rem' })}>
                  DIGEST
                </Typography>
              </Grid>
              <Grid item xs={6} md={3} sx={(theme) => ({ color: theme.palette.secondary.dark, fontSize: '1rem' })}>
